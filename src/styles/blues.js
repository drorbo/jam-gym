...
import { BLUES_FIGURE_STEPS, figureBar, oddFigure } from './figures.js';
...
function bass(ctx) {
  if (ctx.meter.id !== '4/4') return oddFigure(ctx) ?? oddBass(ctx, 'blues');
  ...
  if (BLUES_FIGURE_STEPS[pattern]) return figureBar(ctx, BLUES_FIGURE_STEPS[pattern]);
